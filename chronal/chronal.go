...
func main() {
  ...
  for found == false {
    count++
    fmt.Printf("This is round: " + strconv.Itoa(count) + "\n")
    for j := 0; j < 1028; j++ {
      sum += arr[j]
      if Contains(arr2[:], sum) {
        fmt.Printf(strconv.Itoa(sum) + "\n")
        defer fmt.Printf(strconv.Itoa(sum)+"}}}}}\n")
